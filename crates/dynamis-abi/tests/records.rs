...
fn dof_flag_helpers_round_trip_every_index() {
    let mut flags = 0;
    for index in 0..6 {
        flags = dynamis_abi::set_dof_locked(flags, index, true);
        flags = dynamis_abi::set_dof_limited(flags, index, true);
        flags = dynamis_abi::set_dof_driven(flags, index, true);
    }
    for index in 0..6 {
    ...
    for index in 0..6 {
        flags = dynamis_abi::set_dof_locked(flags, index, false);
        assert!(!dof_locked(flags, index));
        assert!(dof_limited(flags, index));
        assert!(dof_driven(flags, index));
    }
...
fn soft_particle_packs_its_scalar_lanes() {
    ...
    assert_eq!(particle.velocity, [7.0, 8.0, 9.0, 0.5]);
